    ...
    <header className="border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
      <div className="mx-auto max-w-6xl flex items-center justify-between gap-2">
        <Link to="/dashboard" className="flex items-center gap-2 font-bold shrink-0">
          ...
          <span className="hidden sm:inline">CALZADO J&amp;R</span>
        </Link>
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          <span className="text-sm text-gray-600 dark:text-gray-300 truncate max-w-[120px] sm:max-w-none">{user?.full_name}</span>
          <button className="text-sm text-red-600 whitespace-nowrap" onClick={logout}>Cerrar sesión</button>
        </div>
      </div>
    </header>
  );
